...
class TypeInference {
     ...
     */
    fun inferProgram(program: Program): Pair<TypeEnv, TypedProgram> {
        ...
        val statements = mutableListOf<TypedStatement>()
        // Second pass: generate and solve constraints
        var typeEnv = symbolTable.toTypeEnv()

        program.statements.forEach { stmt ->
            val (_, constraints, typeBindings) = constraintGenerator.generateConstraints(stmt, typeEnv)
            ...
            val newStatement = decorateStatement(stmt, resolvedBindings)
            statements.add(newStatement)

            if (newStatement is TypedLetDecl) {
                newStatement.declarations.forEach { decl ->
                    val name = decl.name
                    typeEnv = typeEnv.extend(mapOf(name to TypeScheme(emptyList(), decl.type)))
                }
            }
        }
        ...
        return Pair(
            typeEnv,
            TypedProgram(statements, if (statements.isEmpty()) NamedType.NOTHING else statements.last().type)
        )
    }
    ...
    private fun decorateSingleLetDecl(
        ...
        val typedParams = if (decl.parameterList != null && decl.parameterList.paramList != null) {
            ...
            allParams.map { param ->
                val paramType =
                    typeBindings[param] ?: throw TypeError("No type binding for parameter ${param.name.lexeme}")
    ...
    private fun decorateExpression(expr: Expression, typeBindings: Map<AstNode, Type>): TypedExpression {
        val exprType = typeBindings[expr] ?: throw TypeError("No type binding for expression $expr")
    ...
    private fun decoratePrimaryExpr(expr: PrimaryExpr, typeBindings: Map<AstNode, Type>): TypedPrimaryExpr {
        val exprType = typeBindings[expr] ?: throw TypeError("No type binding for primary expression $expr")
    ...
    private fun decorateSuffix(suffix: ExpressionSuffix, typeBindings: Map<AstNode, Type>): TypedExpressionSuffix {
        val suffixType = typeBindings[suffix] ?: throw TypeError("No type binding for suffix $suffix")
    ...
    private fun decoratePattern(pattern: Pattern, typeBindings: Map<AstNode, Type>): TypedPattern {
        val patternType = typeBindings[pattern] ?: throw TypeError("No type binding for pattern $pattern")
    ...
    private fun decorateTypeExpr(typeExpr: TypeExpr, typeBindings: Map<AstNode, Type>): TypedTypeExpr {
        val resolvedType = typeBindings[typeExpr] ?: throw TypeError("No type binding for type expression $typeExpr")
